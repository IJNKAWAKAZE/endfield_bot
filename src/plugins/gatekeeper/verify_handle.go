...
func VerifyMember(message *tgbotapi.Message) {
	...
	// 限制用户发送消息
	_, err := bot.Endfield.RestrictChatMember(chatId, userId, tgbotapi.NoMessagesPermission)
	if err != nil {
	...
	sendPhoto := tgbotapi.NewPhoto(chatId, tgbotapi.FileBytes{Bytes: utils.GetImg(correct.Illustration)})
	...
	if err != nil {
		...
		bot.Endfield.RestrictChatMember(chatId, userId, tgbotapi.AllPermissions)
		verifySet.checkExistAndRemove(userId, chatId)
		return
	}
...
func unban(chatId, userId int64) {
	...
	bot.Endfield.UnbanChatMember(chatId, userId)
}
...
func verify(chatId int64, userId int64, messageId int, joinMessageId int) {
	...
	bot.Endfield.Send(delJoinMessage)
	// 删除入群验证消息
